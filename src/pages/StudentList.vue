    ...
    <q-card>
      ...
      <q-card-section>
        ...
        <q-markup-table>
          ...
          <tbody>
              <tr v-for="(item, index) in dataListstu" :key="index">
                <td>
                <q-avatar size="125px">
        <img :src="item.image.path">
      </q-avatar>
      </td>
                <td class="text-left">{{ item.std_code}}</td>
                <td class="text-right">{{item.name}}</td>
                <td class="text-right">{{item.surname}}</td>
                <td class="text-right">{{item.birth_date}}</td>
                <td>
                  <q-btn :icon="biPencilFill" flat :to="`student-form/${item.id}`">
